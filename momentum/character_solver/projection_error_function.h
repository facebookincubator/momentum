...
MOMENTUM_FWD_DECLARE_TEMPLATE_STRUCT(ProjectionConstraintData);
MOMENTUM_FWD_DECLARE_TEMPLATE_CLASS(ProjectionErrorFunction);

// momentum Projection constraint that supports our camera model.
template <typename T>
...
class ProjectionErrorFunctionT : public momentum::SkeletonErrorFunctionT<T> {
 public:
  ...
  void addConstraint(const ProjectionConstraintDataT<T>& cons);
 ...
 protected:
  std::vector<ProjectionConstraintDataT<T>> _constraints;
...
// Build a projection matrix such that the error is measured in radians, taking
// the transform taking a worldspace point to camera-local space and a direction
